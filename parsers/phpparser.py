...
php_def_pattern = re.compile("^\<\?(php)")
php_license_pattern = re.compile("^\/\*\s*\*\s*(Copyright)\s*")
...
def parse(arg, cmd_options):
    ...
    with open(arg, 'r') as read_file:
        ...
        if php_license_pattern.match(license_lines):
            return
        else:
            read_file.seek(-read_file.tell(), os.SEEK_CUR)

        with open(tmp_file.name, 'a') as write_file:
            # check if we have the default PHP opening line
            first_line = read_file.readline()
            if php_def_pattern.match(first_line):
